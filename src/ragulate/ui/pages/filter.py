...
@st.cache_data
def get_metadata_filter_options(recipes: List[str], dataset: str, timestamp: int) -> Dict[str, Set[Any]]:
...
metadata_options = get_metadata_filter_options(recipes=recipes, dataset=dataset, timestamp=0)

for key, options in metadata_options.items():
    options = sorted(list(options))
    options.insert(0, SELECT_ALL_TEXT)
    st.selectbox(label=key, options=options, key=f"select_filter_{key}")
